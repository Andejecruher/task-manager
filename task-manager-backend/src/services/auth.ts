...
import { AuthError, COMPANY_SLUG_REGEX, DeviceInfo, EMAIL_REGEX, LoginDTO, LoginResponse, RegisterDTO } from '../types';
...
export class AuthService {
     ...
     */
    async register(data: RegisterDTO, deviceInfo: DeviceInfo): Promise<LoginResponse> {
        ...
        return await db.transaction(async (client) => {
            try {
                ...
                if (existingUser.length > 0) {
                    throw new AuthError('El email ya está registrado', 'EMAIL_ALREADY_EXISTS', 409);
                }

                // 2. Verificar que el slug de compañía esté disponible
                const existingCompany = await client.query(`SELECT id FROM companies WHERE slug = $1`, [
                    data.companySlug,
                ]);

                if (existingCompany.length > 0) {
                    throw new AuthError('El nombre de empresa ya está en uso', 'COMPANY_SLUG_EXISTS', 409);
                }
           ...
           VALUES ($1, 'Mi Workspace', 'default', 'Workspace principal', $2)`,
                ...
                const { token: verificationToken } = tokenService.generateEmailVerificationToken(
                    user.id,
                    company.id,
                );
   ...
   * Login de usuario
   */
    async login(data: LoginDTO, deviceInfo: any, ipAddress?: string): Promise<LoginResponse> {
        // Validar datos
        if (!data.email || !data.password || !data.companySlug) {
            throw new AuthError('Email, contraseña y empresa son requeridos', 'VALIDATION_ERROR', 400);
        }
        ...
        try {
         ...
         WHERE email = $1 AND company_id = $2 AND deleted_at IS NULL`,
            ...
            if (user.locked_until && new Date(user.locked_until) > new Date()) {
                throw new AuthError('Cuenta bloqueada temporalmente', 'ACCOUNT_LOCKED', 423);
            }
  ...
  */
    async getSessions(userId: string, companyId: string, currentSessionId?: string) {
        return await sessionService.getUserSessions(userId, companyId, currentSessionId);
    }
    ...
    private validateRegistrationData(data: RegisterDTO): void {
        ...
        if (!data.companySlug || !COMPANY_SLUG_REGEX.test(data.companySlug)) {
            errors.push('Slug de empresa inválido. Solo letras minúsculas, números y guiones');
        }
    ...
    private async incrementFailedAttempts(userId: string, companyId: string): Promise<void> {
        try {
            await db.query(
                `UPDATE users 
    ...
    private async resetFailedAttempts(userId: string, companyId: string): Promise<void> {
        try {
            await db.query(
                `UPDATE users 
