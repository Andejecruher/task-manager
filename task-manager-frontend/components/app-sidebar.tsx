...
export function AppSidebar() {
    ...
    const isActive = (href: string) => {
        if (href === `/${user?.company?.slug}/workspaces`) return pathname.startsWith(`/${user?.company?.slug}/workspaces`)
    ...
    return (
        <aside className="flex flex-col w-64 border-r border-border bg-card h-screen sticky top-0 shrink-0">
            <div className="p-6 border-b border-border">
                ...
                {user?.company?.slug && (
                    <p className="text-xs text-muted-foreground mt-2 font-mono">{user.company.slug}</p>
                )}
            </div>
                    ...
                    <div className="flex items-center gap-3 p-3 rounded-lg bg-muted">
                        <Avatar className="h-9 w-9">
                            <AvatarFallback className="bg-blue-600 text-white text-sm">
                                {user.user.fullName?.charAt(0).toUpperCase() || user.user.email.charAt(0).toUpperCase()}
                            </AvatarFallback>
                        </Avatar>
                        <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium truncate">{user.user.fullName || user.user.email}</p>
