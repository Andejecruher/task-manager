...
export class TokenService {
    ...
    generateAccessToken(payload: Omit<JwtPayload, 'iat' | 'exp'>): {
        ...
        try {
            ...
            throw new AuthError('Error generando token', 'TOKEN_GENERATION_ERROR', 500);
        }
    }
    ...
    generateRefreshToken(payload: RefreshTokenPayload): {
        ...
        try {
            ...
            throw new AuthError('Error generando refresh token', 'REFRESH_TOKEN_GENERATION_ERROR', 500);
        }
    }
    ...
    verifyAccessToken(token: string): JwtPayload {
        try {
            ...
            throw new AuthError('Error verificando token', 'TOKEN_VERIFICATION_ERROR', 500);
        }
    }
    ...
    verifyRefreshToken(token: string): RefreshTokenPayload {
        try {
            ...
            if (error.name === 'TokenExpiredError') {
                throw new AuthError('Refresh token expirado', 'REFRESH_TOKEN_EXPIRED', 401);
            }
            throw new AuthError('Refresh token inválido', 'INVALID_REFRESH_TOKEN', 401);
        }
    }
     ...
     */
    generateResetPasswordToken(userId: string, companyId: string): {
        token: string;
        expiresAt: Date;
    } {
    ...
    async verifyResetPasswordToken(token: string): Promise<{
        ...
        if (!stored) {
            throw new AuthError('Token inválido o expirado', 'INVALID_RESET_TOKEN', 400);
        }
     ...
     */
    generateEmailVerificationToken(userId: string, companyId: string): {
        token: string;
        expiresAt: Date;
    } {
        const token = crypto.randomBytes(32).toString('hex');
    ...
    async verifyEmailVerificationToken(token: string): Promise<{
        ...
        if (!stored) {
            throw new AuthError('Token inválido o expirado', 'INVALID_VERIFICATION_TOKEN', 400);
        }
