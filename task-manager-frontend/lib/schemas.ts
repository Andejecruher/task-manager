...
// ── Enums ────────────────────────────────────────────────────────────────────
export const taskStatusEnum = z.enum(["todo", "in_progress", "in_review", "done"] as const)
export const priorityEnum = z.enum(["low", "medium", "high", "urgent"] as const)
...
const companySlugSchema = z
    ...
    .transform((val) => {
        const slug = val.trim().toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '').replace(/^-+|-+$/g, '');
