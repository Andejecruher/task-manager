...
interface UserCreationAttributes extends Optional<UserAttributes, 'id' | 'email_verified' | 'mfa_enabled' | 'failed_login_attempts' | 'permissions' | 'is_active' | 'is_onboarded' | 'timezone' | 'locale'> { }
...
class User extends Model<UserAttributes, UserCreationAttributes> implements UserAttributes {
    ...
    toJSON() {
        const { password_hash, mfa_secret, ...sanitizedValues } = this.get({ plain: true }) as UserAttributes;
...
User.init(
    ...
    },
    {
        ...
        defaultScope: {
            attributes: {
                exclude: ['password_hash', 'mfa_secret', 'mfa_enabled', 'failed_login_attempts', 'locked_until', 'last_login_at', 'gdpr_consent_at', 'data_retention_until', 'created_at', 'updated_at', 'deleted_at']
            }
        },
        scopes: {
            withPassword: {
